...
public unsafe partial struct FFXIVReplay
{
    ...
    public struct Header
    {
        ...
        public Lumina.Excel.Sheets.ClassJob LocalPlayerClassJob =>
            DalamudApi.DataManager.GetExcelSheet<Lumina.Excel.Sheets.ClassJob>()!.GetRowOrDefault(jobs[playerIndex])
            ?? DalamudApi.DataManager.GetExcelSheet<Lumina.Excel.Sheets.ClassJob>()!.GetRow(0);
        ...
        public IEnumerable<Lumina.Excel.Sheets.ClassJob> ClassJobs => Enumerable.Range(0, 8)
            ...
            .Select(id => DalamudApi.DataManager.GetExcelSheet<Lumina.Excel.Sheets.ClassJob>()!.GetRowOrDefault(id)
                ?? DalamudApi.DataManager.GetExcelSheet<Lumina.Excel.Sheets.ClassJob>()!.GetRow(0));
    }
